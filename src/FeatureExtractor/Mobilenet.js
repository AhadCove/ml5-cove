...
import { checkLocalUrl } from '../utils/generalUtils';
...
load(filesOrPath = null) {
    return new Promise(async (resolve, reject) => {
      ...
      } else if (checkLocalUrl(filesOrPath)) {
        try {
          const ml5Specs = JSON.parse(localStorage.getItem('ml5Specs'));
          this.mapStringToIndex = ml5Specs.mapStringToIndex;
        } catch (e) {
          console.log('Could not find ml5specs');
          reject(new Error('No ml5 specs'));
        }
  ...
  async save(callback, name) {
    ...
    if (!checkLocalUrl(name)) {
      await this.jointModel.save(name);

        if (callback) {
          callback();
        }
    } else {
        localStorage.setItem('ml5Specs', JSON.stringify({ mapStringToIndex: this.mapStringToIndex }));
  ...
  //       if (callback) {
  //         callback({ weights: data.weightData, model: { ...this.weightsManifest } });
  //       }
  //     }));
  //   } else {
  //       localStorage.setItem('ml5Specs', JSON.stringify({ mapStringToIndex: this.mapStringToIndex }));
